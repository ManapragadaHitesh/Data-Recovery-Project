...
# Usage python ntfs_pattern.py H_image.dd --pattern "*.txt" --outdir recovered
...
def main(argv):
    image_path = input("Enter DD filename:\n")
    lst = [image_path, '--pattern', '*.*', '--outdir', 'recovered']
    args = parse_args(lst) #argv
    #print(argv)
    print(args)
    f = open(args.disk, 'rb')
